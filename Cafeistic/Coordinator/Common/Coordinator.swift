...
class Coordinator: NSObject {
    ...
    init(router: Router, navigationType: NavigationType) {
        ...
        if case .newFlow = navigationType {
            router.setRoot(root)
        }
    }
    ...
    func setRootChild(coordinator: Coordinator, hideBar: Bool = false) {
        addChild(coordinator)
        router.setRoot(coordinator) { [weak self, weak coordinator] in
